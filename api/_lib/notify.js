...
function buildLeadEmailHtml(input) {
  ...
  const eventType = input.eventType === 'update' ? 'update' : 'new';
  const eventTitle = eventType === 'update' ? 'Actualización del lead' : 'Nuevo lead recibido';
  ...
  return (
    ...
    '<div style="font-size:19px;font-weight:700;color:#FFFFFF;margin-top:14px;">' + eventTitle + '</div>' +
    '</td></tr>' +
...
function buildWhatsAppText(input) {
  ...
  const eventType = input.eventType === 'update' ? 'update' : 'new';
  const lines = [eventType === 'update' ? 'Actualización del lead desde JB TECH AI:' : 'Nuevo lead desde JB TECH AI:', ''];
